...
extern unsigned char * load_srd_kernel_base;		//CSFB SRD
...
boolean userDataBackUpBadCalc(void)
{
	...
	srd_bytes_pos_in_emmc = (pMisc_part->dfirstsec)*512 + PTN_SRD_BACKUP_POSITION_IN_MISC_PARTITION;

	printk(KERN_ERR "[SRD BACKUP] MISC dfirstsec = %d", pMisc_part->dfirstsec);
...
===========================================================================*/
void diag_userDataBackUp_entrySet(srd_req_type *req_pkt, srd_rsp_type *rsp_pkt, script_process_type MODEM_MDM )
{
...
#include <mach/LS696_nv_backup_list.h>
void diag_userDataBackUp_data(srd_req_type *req_pkt, srd_rsp_type *rsp_pkt)
{
	int i;
...
void diag_extraNv_entrySet(srd_req_type *req_pkt, srd_rsp_type *rsp_pkt)
{
...
void diag_PRL_entrySet(srd_req_type *req_pkt, srd_rsp_type *rsp_pkt)
{
...
===========================================================================*/
 void diag_SRD_Init(srd_req_type *req_pkt, srd_rsp_type *rsp_pkt)
{
	int nPartionCount;
	rsp_pkt->header.cmd_code = req_pkt->header.cmd_code;
	rsp_pkt->header.sub_cmd = req_pkt->header.sub_cmd;
  ...
  if (nNam >= 2)
  {
    for(i=0; i<=nNam-1; i++)
    {
      ...
      if(out_len!=0)   //out len is zero means not active nv or fail
      {
        ...
        memcpy( ((load_srd_kernel_base)+ud_page_index*256),nv_item_buffer,sizeof(nv_item_buffer));
...
#endif //#ifdef CONFIG_LGE_DIAG_SRD  //kabjoo.choi
...
boolean userWebDnBadCalc(void)
{
	...
	web_status_bytes_pos_in_emmc = (pMisc_part->dfirstsec)*512+PTN_SRD_BACKUP_POSITION_IN_MISC_PARTITION; // TBD
