...
int lg_get_usb_unlock_fail_cnt(void)
{
	struct oem_rapi_client_streaming_func_arg arg;
	struct oem_rapi_client_streaming_func_ret ret;
	byte unlock_fail_cnt = 0x0;
	unsigned int out_len = 0xFFFFFFFF;	
	byte temp = 0;

	Open_check();

	arg.event = LG_FW_GET_USB_UNLOCK_FAIL_CNT;
	arg.cb_func = NULL;
	arg.handle = (void*) 0;
	arg.in_len = sizeof(temp);
	arg.input = &temp;
...
//	arg.input = NULL;
	...
	unlock_fail_cnt = *ret.output;
	out_len = *ret.out_len;

	return (int)unlock_fail_cnt;
}
EXPORT_SYMBOL(lg_get_usb_unlock_fail_cnt);
...
EXPORT_SYMBOL(lg_set_usb_unlock_fail_cnt);
#endif  

#ifdef CONFIG_LGE_DIAG_USB_ACCESS_LOCK_SHA1_ENCRYPTION
void lg_get_usb_lock_key(char * usb_lock_key)
{
	struct oem_rapi_client_streaming_func_arg arg;
	struct oem_rapi_client_streaming_func_ret ret;
	...
	arg.event = LG_FW_GET_USB_LOCK_KEY;
	arg.cb_func = NULL;
	arg.handle = (void*) 0;
	arg.in_len = sizeof(temp);
	arg.input = temp;
...
//	arg.input = NULL;
	...
	if (ret.out_len && ret.output)
	{
		out_len = *ret.out_len;
		memcpy(usb_lock_key, ret.output, out_len);
		
		kfree(ret.output);
		kfree(ret.out_len);
	}
	else
		memset(usb_lock_key, 0x0, 6);

	return;
}
EXPORT_SYMBOL(lg_get_usb_lock_key);
...
EXPORT_SYMBOL(lg_set_usb_lock_key);
#endif

#ifdef CONFIG_LGE_DIAG_USBLOCK_EFS_SYNC
int lg_get_usblock_efs_sync_result(void)
{
	struct oem_rapi_client_streaming_func_arg arg;
	struct oem_rapi_client_streaming_func_ret ret;
	byte efs_sync_result = 0xFF;
	unsigned int out_len = 0xFFFFFFFF;
	int rc = -1;	
	byte temp = 0;
	...
	arg.event = LG_FW_GET_USBLOCK_EFS_SYNC_RESULT;
	arg.cb_func = NULL;
	arg.handle = (void*) 0;
	arg.in_len = sizeof(temp);
	arg.input = &temp;
	...
	efs_sync_result = *ret.output;
	out_len = *ret.out_len;

	return efs_sync_result;
}
EXPORT_SYMBOL(lg_get_usblock_efs_sync_result);
#endif  
