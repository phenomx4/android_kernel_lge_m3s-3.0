...
static struct panic_log_dump *panic_dump_log = NULL;
...
static int panic_handled_once = 0;
		...
		void *ptr)
{
	...
	if(panic_handled_once == 0)
		panic_handled_once++;
	else
	{
		printk(KERN_ERR "%s already handled!!!\n", __func__);
		return 0;
	}
...
#if !defined (CONFIG_VT_CONSOLE)
    vc_cons[PANIC_DUMP_CONSOLE].d = vc = kzalloc(sizeof(struct vc_data), GFP_NOWAIT);
#else
	vc = vc_cons[PANIC_DUMP_CONSOLE].d;
#endif
	display_buffer =
...
static int __devinit lge_panic_handler_probe(struct platform_device *pdev)
{
	int ret = 0;
