...
u8 QTR_cal_data[QTR_CAL_MAX][4] = {
    {0x0C, 0xFF,0xFF,0xFF}, //QTR_EARPIECE
...
void set_ampcal_data(void)
{
  ...
  while ((token = strtok(NULL, separator)) != NULL)
  {
    ...
    amp_cal_lodata[nIndex] = (char)strtol(token, NULL, 16);
     
    MM_INFO("AMP CHANGE %s -> %x\n", token, amp_cal_lodata[nIndex]);
...
static long audio_misc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
        ...
        case SET_AUDIO_CAL:
		  ...
		  {
		          MM_INFO("ioctl arg else = %d\n", lb_path);
		   	MM_INFO("SET_AUDIO_CAL \n");
			  set_QTRcal_data();
			  if (bReadCalData) {
					set_ampcal_data();
					set_voicecal_data();
	 ...
	 MM_INFO("AUDIO_START_VOICE  START = %d\n", lb_path);
            if (copy_from_user(&lb_path,(void *)arg,sizeof(int)))
        		return 0;
        	MM_INFO("ioctl arg = %d\n", lb_path);
			...
			MM_INFO("AUDIO_START_VOICE   rc = %d\n", lb_path);
		    if (rc < 0)
			    MM_ERR("failed to do loopback\n");
		    else
