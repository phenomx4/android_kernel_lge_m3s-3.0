...
// rpc response for write_iovec is normally come down within 1 sec, so delayed work queue will be set to 5 sec 
#define RMT_STORAGE_DELAYED_WORK_TIME msecs_to_jiffies(5000)
		...
		char *buf)
{
	...
	return snprintf((char *)buf, (size_t)PAGE_SIZE, "current value : %d\n0: enable ioctl response\n1: disable ioctl response\n", get_rmt_storage_kill_test_flag());
}
