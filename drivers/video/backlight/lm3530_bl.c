...
/* LGE_CHANGE
  * LM3530 has two parts of LEDs(Main and Sub)
  * Added some definitions and modified I2C write command to control both Main and Sub LEDs.
  * Added 'LM3530BL_REG_BLS', changed members of structure 'lm3530_reg_addrs'
  * and modified 'lm3530_write' to control two registers(LM3530BL_REG_BLM and LM3530BL_REG_BLS)
  * 2010-04-22, minjong.gong@lge.com
  */

/* LGE_CHANGE
  * If MEQS bit in LM3530BL_REG_BLM is set, we don't need to write command to LM3530BL_REG_BLS.
  * So modify command array for LM3530 and related functions. 
  * And change default brightness and maximum brightness.
  * 2010-05-18, minjong.gong@lge.com
  */
//#define LCD_BRIGHTNESS_TUNING 

#define LCD_LED_MAX  0x7D // 0x7B   2012.07.23 yunjeong.kang MAX 400nit
#define LCD_LED_MIN 0

#if 1 // uhsooji_temp, same to M3
#define DEFAULT_BRIGHTNESS 0x6A
 
#define LM3530_DIM_BRIGHTNESS	0x3C // Dim
#define LM3530_MIN_BRIGHTNESS	0x56 // 10% full 22.5mA
#define LM3530_DEFAULT_BRIGHTNESS	0x6A // 42%  full 22.5mA

#define LM3530_DIM_VALUE_SETTINGS 10
#define LM3530_MIN_VALUE_SETTINGS 30
#define LM3530_DEFAULT_VALUE_SETTINGS 134
#else
//#define DEFAULT_BRIGHTNESS 0x32
#define DEFAULT_BRIGHTNESS 0x7A
 
#define LM3530_MIN_BRIGHTNESS	0x56 //10% full 22.5mA
#define LM3530_DEFAULT_BRIGHTNESS	0x7A //75%  full 22.5mA
#endif

#define LM3530BL_REG_BLS   0x04  /* Register address for Main BL brightness */
#define LM3530BL_REG_FADE	0x07  /* Register address for Backlight Fade control */
...
#ifdef CONFIG_LGE_LCD_DISPLAY_TIMER  
	atomic_t lcd_event_handled;  
#endif  

#ifdef LCD_BRIGHTNESS_TUNING  
// /sys/class/leds/lcd-backlight  
 static unsigned int brightness_setting = 0x56; //10% 22.5mA  
 #endif  
...
#define CONST_BL_VALUE  0x3C  //0x35
extern int get_charger_id_type(void);
#endif
...
static struct lm3530_ctrl_tbl lm3530_normal_tbl[] = {
	{ LM3530BL_REG_GENERAL_CONFIGURATION, 0x15 },   // Full Scale Current Select 26mA, exponential
	{ 0xFF, 0xFE }	 /* end of command */
};

/* Set to ALC mode */
static struct lm3530_ctrl_tbl lm3530_alc_tbl[] = {
	// LM3530 has no ALC mode !!
...
static struct lm3530_driver_data *lm3530_dev = NULL;
 ...
 ********************************************/
#ifdef LCD_BRIGHTNESS_TUNING  
 static ssize_t brightness_save_tuning(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)	
{  
	   sscanf(buf, "%x", &brightness_setting);  
	   printk("brightness_save_tuning 0x%x \n", brightness_setting);  
	 	  
	   return count;  
 }  
 static ssize_t brightness_show_tuning(struct device *dev, struct device_attribute *attr,char *buf)  
{  
	   return sprintf(buf, "0x%x\n", brightness_setting);	
}  
	 
 DEVICE_ATTR(tuning, 0664 , brightness_show_tuning, brightness_save_tuning);	
#endif  
...
static int lm3530_setup_version(struct lm3530_driver_data *drvdata)
{
	...
	drvdata->reg_addrs.bl_s = LM3530BL_REG_BLS;
	drvdata->reg_addrs.fade = LM3530BL_REG_FADE;
...
{
/* LGE_CHANGE.
  * Do not initialize lm3530 when system booting. The lm3530 is already initialized in oemsbl or LK !!
...
//LGE_CHANGE_S [jinhwan.do][2012.03.07]TestMode patch source merge from LS696 Model [End]
	...
	if (drvdata->state == POWEROFF_STATE) {
		 ...
		 * If it is called two times when the previous state of LM3530 is POWEROFF_STATE, it causes malfucction.
...
#ifdef LCD_BRIGHTNESS_TUNING  
		  lm3530_write(drvdata->client, drvdata->reg_addrs.bl_m, brightness_setting);  
		 drvdata->intensity = brightness_setting;  
#else  
...
{	
	...
	printk("[SLEEP WAKE UP] lm3530_late_resume ...\n");
...
{
	/* LGE_S,[US730],lcd backlight register show crash issue */
	//struct lm3530_driver_data *drvdata = dev_get_drvdata(dev);
	struct lm3530_driver_data *drvdata = dev_get_drvdata(dev->parent);
	/* LGE_E,[US730],lcd backlight register show crash issue */
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "[GP(0x10)] = 0x%x\n", val);
	lm3530_read(drvdata->client, 0x30, &val);
	len += snprintf(buf + len, PAGE_SIZE - len, "[BRR(0x30)] = 0x%x\n", val);
	lm3530_read(drvdata->client, 0xA0, &val);
	len += snprintf(buf + len, PAGE_SIZE - len, "[BRT(0xA0)] = 0x%x\n", val);	
#else
	len += snprintf(buf,       PAGE_SIZE,       "\nLM3530 Registers is following..\n");
...
#if 1 //uhsooji_temp, same to M3
	if (value <= LM3530_DIM_VALUE_SETTINGS)
	{
	 next = value * LM3530_DIM_BRIGHTNESS / LM3530_DIM_VALUE_SETTINGS;
	}
	else if(value > LM3530_DIM_VALUE_SETTINGS && value <= LM3530_MIN_VALUE_SETTINGS)
	{
	 next = (value - LM3530_DIM_VALUE_SETTINGS) * (LM3530_MIN_BRIGHTNESS - LM3530_DIM_BRIGHTNESS) / (LM3530_MIN_VALUE_SETTINGS - LM3530_DIM_VALUE_SETTINGS) + LM3530_DIM_BRIGHTNESS;
	}
...
//	printk("===============================================================input brightness US value = %d   BSPvalue=%d]\n", value , next);
...
{
	struct m3s_backlight_platform_data *pdata;
...
#ifdef LCD_BRIGHTNESS_TUNING 
		err = device_create_file(drvdata->led->dev, &dev_attr_tuning); 
#endif 
}
#endif
...
void lm3530_backlight_on(void)
{
	printk("%s()\n", __func__);

	lm3530_wakeup(lm3530_dev);

	return;
}
EXPORT_SYMBOL(lm3530_backlight_on); //using testmode only...

void lm3530_backlight_off(void)
{
	printk("%s()\n", __func__);

	lm3530_sleep(lm3530_dev);
	mdelay(1);

	return;
}
EXPORT_SYMBOL(lm3530_backlight_off); //using testmode only...
