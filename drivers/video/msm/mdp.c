...
static int mdp_lut_init_update_lcdc(void )
{
	...
	printk("### mdp_lut_init_update_lcdc++: mdp_lut_i = %d=========================1\n", mdp_lut_i);
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
//	ret = mdp_lut_hw_update(cmap);
      printk("### mdp_lut_init_update_lcdc++: mdp_lut_i = %d==========================2\n", mdp_lut_i);

  ret = mdp_lut_initial_hw_update();
  printk("### mdp_lut_init_update_lcdc++: mdp_lut_i = %d==========================3\n", mdp_lut_i);
	if (ret) {
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
		return ret;
	}
