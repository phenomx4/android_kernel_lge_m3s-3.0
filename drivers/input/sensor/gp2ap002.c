...
//You can control debug log by using debug_mask node
//You'd better to use debug_mask node, not to undefine below macro
#define PROXIMITY_DEBUG_PRINT 
#define PROXIMITY_ERROR_PRINT
...
static irqreturn_t gp2ap_irq_handler(int irq, void *dev_id)
{
	...
	printk(KERN_INFO "%s: pdev->debounce=%d\n",
		__func__, pdev->debounce);	
	delay = msecs_to_jiffies(pdev->debounce);
	printk(KERN_INFO "%s: delay=%d\n",
		__func__, (int)delay);	
...
gp2ap_work_func(struct work_struct *work)
{
	...
	if (dev->methods) {  	/* Interrupt mode */
		...
		PROXD("Normal mode\n");
		if (gpio_get_value((unsigned)proxi_pdata->irq_num) == 1) /* FAR */
	...
	if (GP2AP_DEBUG_FUNC_TRACE & gp2ap_debug_mask)
		PROXD("dev->last_vout= %d, vo_data=%d\n", dev->last_vout, vo_data);
...
gp2ap_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	static u32 old_status =1; /*FAR*/
	static u32 new_status;

/*	if (GP2AP_DEBUG_FUNC_TRACE & gp2ap_debug_mask)
		PROXD("entry, status=%d\n", !(pdev->last_vout));
*/
	new_status= !(pdev->last_vout);
	if(new_status != old_status){
		PROXD("old_status=%d, new_status=%d\n", old_status, new_status);
		old_status= new_status;
	}
...
static ssize_t
gp2ap_pdata_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct proximity_gp2ap_device *pdev = i2c_get_clientdata(client);

	return snprintf(buf, PAGE_SIZE,
		"op_mode=%d cycle=%d methods=%d debounce=%d\n",
		pdev->op_mode, pdev->cycle, pdev->methods, pdev->debounce);
}
...
static struct device_attribute gp2ap_device_attrs[] = {
	...
	__ATTR(lge_pdata, S_IRUGO | S_IWUSR, gp2ap_pdata_show, NULL),
};
...
/* LGE_CHANGE_START[hyeongnam.jang@lge.com, 2012-01-25]*/
...
//	i2c_set_clientdata(client, gp2ap_pdev);

/* LGE_CHANGE_END[hyeongnam.jang@lge.com, 2012-01-25]*/
	...
	/*TEMP IMPLEMENT. START*/
	/*
	printk(KERN_INFO "%s: gp2ap_pdev->debounce=%d\n",
		__func__, gp2ap_pdev->debounce);
	if(gp2ap_pdev->debounce ==0){
		gp2ap_pdev->debounce= 100;	//ms unit
	}
	*/
	/*TEMP IMPLEMENT. END*/
...
/* BELOW CODE IS OLD CODE */
	for (i = 0; i < ARRAY_SIZE(gp2ap_device_attrs); i++) {
		...
		if (ret) {
			printk(KERN_INFO "[D] %s, ret=%d\n", __func__, ret);
			goto err_device_create_file;
		}
	}

/* BELOW CODE IS NEW CODE(INVALID CODE) */
	/*
	ret = sysfs_create_group(&(gp2ap_pdev->input_dev->dev.kobj),
		&gp2ap_attribute_group);
	printk(KERN_INFO "[D] %s(%d): ret=%d\n", __func__, __LINE__, ret);
