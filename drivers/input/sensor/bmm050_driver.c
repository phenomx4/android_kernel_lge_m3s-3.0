...
/*LGE_CHANGE_S[hyeongnam.jang@lge.com, 2012-02-29] to debug initial non-response problem of magnetic sensor */
/* you can switch on the DEBUG_MODE, by enable below 'define'	*/
//#define	BMM050_DRIVER_DEBUG_MODE	

#ifdef BMM050_DRIVER_DEBUG_MODE
unsigned int lge_bmm_get_op_mode(struct device *dev);
#endif
/*LGE_CHANGE_E[hyeongnam.jang@lge.com, 2012-02-29] to debug initial non-response problem of magnetic sensor */
...
static struct workqueue_struct * bmm050_workqueue;
...
static void bmm_work_func(struct work_struct *work)
{
	...
	/*LGE_CHANGE [2012-02-06]*/
	printk(KERN_INFO "%s(), work_func (x,y,z)=(%d,%d,%d), op_mode=%d\n",__func__,
		client_data->value.datax, client_data->value.datax, client_data->value.datax,
		client_data->op_mode);
	...
	#else
	queue_delayed_work(bmm050_workqueue, &client_data->work, delay);
	#endif
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	/*LGE_CHANGE_S[hyeongnam.jang@lge.com, 2012-03-06] to make log*/
#ifdef BMM050_DRIVER_DEBUG_MODE
	op_mode= lge_bmm_get_op_mode(dev);
	printk(KERN_INFO"%s:%d: op_mode=%x\n", __func__, __LINE__, (int)op_mode);
#endif
	/*LGE_CHANGE_E[hyeongnam.jang@lge.com, 2012-03-06] to make log*/
		...
		if (data) {
			...
			schedule_delayed_work(
					...
					msecs_to_jiffies(atomic_read(
							&client_data->delay)));
			#else
			queue_delayed_work(bmm050_workqueue, &client_data->work,
					msecs_to_jiffies(atomic_read(&client_data->delay)));
			#endif
...
/*LGE_CHANGE_S[hyeongnam.jang@lge.com, 2012-02-22] to make count node for Testmode*/
static ssize_t bmm_show_count(struct device *dev, struct device_attribute *attr, 
	char *buf)
{
	return sprintf(buf, "%d\n",g_count);
}
...
/*LGE_CHANGE_S[hyeongnam.jang@lge.com, 2012-03-20]  */
static ssize_t bmm_store_lge_enable_ta(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long data;
	...
	if (data != client_data->enable) {
		if (data) {
			...
			#endif
			/* late_resume() process */
			/* below 3 lines are come from late_resume()*/
			mutex_lock(&client_data->mutex_power_mode);
...
static ssize_t bmm_show_lge_enable_ta(struct device *dev, struct device_attribute *attr, 
	char *buf)
{
	return bmm_show_enable(dev, attr, buf);
}

/*LGE_CHANGE_E[hyeongnam.jang@lge.com, 2012-03-20]  */
...
static DEVICE_ATTR(count, S_IRUGO|S_IWUSR,
		bmm_show_count, NULL);	
/*LGE_CHANGE_E [hyeongnam.jang@lge.com] */
/*LGE_CHANGE_S[hyeongnam.jang@lge.com, 2012-03-20] to make count node for Testmode*/
static DEVICE_ATTR(lge_enable_ta, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bmm_show_lge_enable_ta, bmm_store_lge_enable_ta);
/*LGE_CHANGE_E[hyeongnam.jang@lge.com, 2012-03-20] to make count node for Testmode*/
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	PNOTICE("i2c_client: %p client_data: %p i2c_device: %p input: %p",
...
unsigned int lge_bmm_get_op_mode(struct device *dev){
	...
	struct input_dev *input = to_input_dev(dev);
	struct bmm_client_data *client_data = input_get_drvdata(input);
	unsigned char op_mode=0xcc;
	unsigned char power_mode;
	const char POWER_OFF= 0x10;
	
	mutex_lock(&client_data->mutex_power_mode);
	BMM_CALL_API(get_powermode)(&power_mode);
	if (power_mode) {
		mutex_lock(&client_data->mutex_op_mode);
		BMM_CALL_API(get_functional_state)(&op_mode);	//set g_op_mode
		mutex_unlock(&client_data->mutex_op_mode);
	} else {
		op_mode = BMM_VAL_NAME(SUSPEND_MODE) | POWER_OFF;
	}
...
#endif /* BMM050_DRIVER_DEBUG_MODE */
/*LGE_CHANGE_S[hyeongnam.jang@lge.com, 2012-02-29] to debug initial non-response problem of magnetic sensor */
...
{
	bmm050_workqueue = create_workqueue("magnetic sensor: bmm050");
	if(!bmm050_workqueue){
		return -ENOMEM;
	}
...
{
	if(bmm050_workqueue){
		destroy_workqueue(bmm050_workqueue);
		bmm050_workqueue = NULL;
	}
